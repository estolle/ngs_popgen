...
cat $LISTtoPROCESS | parallel --no-notice -j 3 "echo {}; clumpify.sh in1=$INPUTFOLDER/{}'_1'.fastq.gz in2=$INPUTFOLDER/{}'_2'.fastq.gz out1=$TMPFOLDER/{}.temp.R1.fastq.gz out2=$TMPFOLDER/{}.temp.R2.fastq.gz dedupe optical dupedist=$OPTICALDIST -Xmx100g passes=1 subs=1 k=31 spantiles=f 2> >(tee $OUTPUTFOLDER/{}.optical_duplicates.log >&1); echo clumpify_done; \
...
#strict filtering (sickle) and moving of files
...
rm -f $TMPFOLDER/{}.temp.R1.fastq.gz $TMPFOLDER/{}.temp.R2.fastq.gz ; echo {}"
...
ls -1 $INPUTFOLDER/*.fastq.gz | rev | cut -d"/" -f1 | cut -d"." -f3- | rev | parallel --no-notice -j 15 "echo {}; fastqc --noextract -f fastq --threads 10 --nogroup -o $INPUTFOLDER/fastqcs/ $INPUTFOLDER/{}.fastq.gz; echo {}"

ls -1 $OUTPUTFOLDER/*.fastq.gz | rev | cut -d"/" -f1 | cut -d"." -f3- | rev | parallel --no-notice -j 15 "echo {}; fastqc --noextract -f fastq --threads 10 --nogroup -o $OUTPUTFOLDER/fastqcs/ $OUTPUTFOLDER/{}.fastq.gz; echo {}"
...
READPAIRSbefore=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "read pairs processed" | cut -d" " -f1)
DEGEN=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "degenerative read pairs" | cut -d"(" -f1 | sed "s/ //g")
QUALFILTERED=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "read pairs filtered out by quality control" | cut -d"(" -f1 | sed "s/ //g")
TOOSHORT=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "short read pairs filtered out after trimming by size control" | cut -d"(" -f1 | sed "s/ //g")
EMPTY=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "empty read pairs filtered out after trimming by size control" | cut -d"(" -f1 | sed "s/ //g")
READPAIRSafter=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "read pairs available; of these" | cut -d"(" -f1 | sed "s/ //g")
READPAIRSafterTRIMMED=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  " trimmed read pairs available after processing" | cut -d"(" -f1 | sed "s/ //g")
READPAIRSafterUNTRIMMED=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  " untrimmed read pairs available after processing" | cut -d"(" -f1 | sed "s/ //g") 
READPAIRSafterPC=$(cat $OUTPUTFOLDER/$SAMPLE.skewer.log | grep  "read pairs available; of these" | cut -d"(" -f2- | cut -d")" -f1| sed "s/ //g")
