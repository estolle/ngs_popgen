...
colnames(df) <- c("SAMPLE","MISSING","HMZREF","HTZ","HMZALT")
...
ylabel <- title
MEDIANMISSING <- df2.summary$MISSING_Median
MEDIANHTZ <- df2.summary$HTZ_Median
MEDIANHMZA <- df2.summary$HMZALT_Median
MEDIANHMZR <- df2.summary$HMZREF_Median
MAX1=max(df2$MISSING)
MAX2=max(df2$HMZALT)
MAX3=max(df2$HMZREF)
MAX4=max(df2$HTZ)
YMAX <- max(MAX1,MAX2,MAX3,MAX4)
YMAX.f=as.integer(format((plyr::round_any(YMAX, 5000, f = ceiling)), scientific=FALSE))

Y1=as.integer(format((plyr::round_any(1*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y2=as.integer(format((plyr::round_any(2*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y3=as.integer(format((plyr::round_any(3*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y4=as.integer(format((plyr::round_any(4*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y5=as.integer(format((plyr::round_any(5*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y6=as.integer(format((plyr::round_any(6*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y7=as.integer(format((plyr::round_any(7*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y8=as.integer(format((plyr::round_any(8*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y9=as.integer(format((plyr::round_any(9*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
Y10=as.integer(format((plyr::round_any(10*(YMAX.f/10), 5000, f = ceiling)), scientific=FALSE))
...
plot1 <-
ggplot(df2, aes(MISSING, desc(SAMPLE.f),fill=factor(as.integer(SAMPLE.f) %% 2))) +
      ...
      geom_vline(xintercept = MEDIANHMZA, linetype = 2, color="black",size=1.0) +
      geom_vline(xintercept = MEDIANHMZR, linetype = 2, color="black",size=1.0) +
      geom_hline(data = hlines_all,aes(yintercept = hlines_all[,1]), size = 0.1, colour = "grey", linetype = 1 ,alpha=0.55) +
    scale_x_continuous(breaks= c(Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8,Y9,Y10,MEDIANMISSING,MEDIANHTZ,MEDIANHMZA,MEDIANHMZR,YMAX.f), labels = function(x) {sprintf("%00d", as.integer(x))}, expand = c(0,(YMAX.f/10)),limit = c(0, YMAX.f)) +
...
geom_point(aes(x = 9*(YMAX.f/10)*1.05, y = as.integer(length(df2$SAMPLE.f)), colour=factor(as.integer(SAMPLE.f) %% 2), fill=factor(as.integer(SAMPLE.f) %% 2)), size = 1,na.rm = FALSE) +
annotate(geom="text", x = 9*(YMAX.f/10)*1.05, y = as.integer(length(df2$SAMPLE.f)), label = "Missing", size = 3, colour = "black",hjust=-0.2, vjust=-0.0,angle=-90) +
geom_point(x = 9*(YMAX.f/10)*1.03, y = as.integer(length(df2$SAMPLE.f)), colour="black", fill="darkgrey", shape=21 , size = 2.5) +
annotate(geom="text", x = 9*(YMAX.f/10)*1.03, y = as.integer(length(df2$SAMPLE.f)), label = "HTZ", size = 3, colour = "black",hjust=-0.2, vjust=-0.0,angle=-90) +
geom_point(x = 9*(YMAX.f/10)*1.00, y = as.integer(length(df2$SAMPLE.f)), colour="white", fill="darkgrey", shape=21, size = 1.5,na.rm = FALSE) +
annotate(geom="text", x = 9*(YMAX.f/10)*1.00, y = as.integer(length(df2$SAMPLE.f)), label = "HMZ-REF", size = 3, colour = "black",hjust=-0.2, vjust=-0.0,angle=-90) +
geom_point(x = 9*(YMAX.f/10)*0.95, y = as.integer(length(df2$SAMPLE.f)), colour="black", fill="darkgrey", shape=21, size = 1.5,na.rm = FALSE) +
annotate(geom="text", x = 9*(YMAX.f/10)*0.95, y = as.integer(length(df2$SAMPLE.f)), label = "HMZ-ALT", size = 3, colour = "black",hjust=-0.2, vjust=-0.0,angle=-90) +
...
geom_point(x = df2$HTZ, y = as.integer(df2$SAMPLE.f), colour="black", fill="darkgrey", shape=21, size = 2.5, na.rm = FALSE) +
geom_point(x = df2$HMZREF, y = as.integer(df2$SAMPLE.f), colour="white", fill="darkgrey", shape=21, size = 1.5, na.rm = FALSE) +
geom_point(x = df2$HMZALT, y = as.integer(df2$SAMPLE.f), colour="black", fill="darkgrey", shape=21, size = 1.5, na.rm = FALSE)
