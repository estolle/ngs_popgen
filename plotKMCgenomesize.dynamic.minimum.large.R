...
#45619475
peak <- max(kmercounts[20:10000,2])
...
#peakregionKmers <- sum(as.numeric(kmercounts[2:(maxKmer+10+maxKmer),1]*kmercounts[2:(maxKmer+10+maxKmer),2]))
peakregionKmers <- sum(as.numeric(kmercounts[minKmer:(maxKmer+10+maxKmer),1]*kmercounts[minKmer:(maxKmer+10+maxKmer),2]))
PEAKSUM <- round((peakregionKmers/maxKmer)/1000000, digits = 2)
#PEAKSUM
#357.3006 Mb
#--> n=1 kmers mostly seq error, but since peak is at n=4, its v close, thus the proximal end of the distribution is inside the n=1 peak, thus this measure is an underestimate
...
#For an inbred line, two set of genomes have mostly identical sequence and the major peak would be diploid (2C) peak. #On the otherhand, for highly heterozygous sample, there would be het$
...
plot(1:(maxKmer*3),dpois(1:(maxKmer*3), maxKmer)*singleC, type = "l", col=3, lty=2,xlab="kmer depth", ylab="kmers")
lines(kmercounts[1:(maxKmer*3),],type="l")
points(kmercounts[2:(maxKmer*3),])
points(kmercounts[maxKmer,],type = "p",pch = 16, col = "black",bg ="black", cex = 3)
#points(kmercounts[2,],type = "p",pch = 16, col = "black",bg ="black", cex = 3)
points(kmercounts[minKmer,],type = "p",pch = 16, col = "black",bg ="black", cex = 3)
points(kmercounts[(maxKmer+10+maxKmer),],type = "p",pch = 16, col = "black",bg ="black", cex = 3)
